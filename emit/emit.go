...
func Typed[T any](q *kernel.EmitterKernel, codec ...kernel.Encoder[T]) (snd chan<- T, dlq <-chan T) {
...
func Event[E swarm.Event[M, T], M, T any](q *kernel.EmitterKernel, codec ...kernel.Encoder[swarm.Event[M, T]]) (snd chan<- swarm.Event[M, T], dlq <-chan swarm.Event[M, T]) {
...
func Bytes(q *kernel.EmitterKernel, codec kernel.Encoder[[]byte]) (snd chan<- []byte, dlq <-chan []byte) {
